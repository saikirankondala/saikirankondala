...
//          wait(2);
    ...
    public void DownloadXlsvFile()
    {
        ...
        if (files != null) {
            for (File file : files)
            {
                if (file.isFile() && file.getName().startsWith("Clu_Read_Only_Users"))
                {
                    System.out.println("Found file: " + file.getAbsolutePath());
                    test().log(Status.PASS, "=====>RA Manager File has been downloaded successfully.");
                }
            }
        }
        else
        {
            ...
            test().log(Status.FAIL, "=====>RA  Manager File download verification failed.");
        }
    }
    ...
    public void VerificationOfXlsvDownloadedFile()
    {
        ...
        if (files != null)
        {
            ...
            {
                test().log(Status.PASS, "=====>RA Manager Pagenation "+ManagerCounts+" count is matched with the XLS File downloaded File Row count "+Manager_XLSVRowCount);
                System.out.println("RA Manager Pagenation "+ManagerCounts+" count is matched with the XLS File downloaded File Row count "+Manager_XLSVRowCount);

            }
            else
            {
                test().log(Status.FAIL, "=====>RA Manager Pagenation "+ManagerCounts+" count is not matched with the XLS File downloaded File Row count "+Manager_XLSVRowCount);
                System.out.println("RA  Manager Pagenation "+ManagerCounts+" count is not matched with the XLS File downloaded File Row count "+Manager_XLSVRowCount);

            }
        }
        else
        {
            test().log(Status.FAIL, "Issue In the RA Mangers Downloaded XLSV file");
