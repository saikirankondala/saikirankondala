...
public class UserManagement_Executive extends Utils
{
    ...
    By Manager=By.xpath("//*[@id=\"executives1\"]");
...
//          wait(2);
    ...
    public void DownloadXlsvFile()
    {
        ...
        if (files != null) {
            for (File file : files)
            {
                if (file.isFile() && file.getName().startsWith("Clu_Executive"))
                {
                    ...
                    test().log(Status.PASS, "=====>Executive File has been downloaded successfully.");
                }
            }
        }
        else
        {
            ...
            test().log(Status.FAIL, "=====>Executive File download verification failed.");
        }
    }
